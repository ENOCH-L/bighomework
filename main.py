...
def say(text):
    # 定义常量
    APP_ID = '24751546'
    API_KEY = 'NgaM4ugKlVXshoeiX6BUC6yG'
    SECRET_KEY = 'rbI23GzaZn9CI174vWg9h4cOv8PnmeA0'
    # 初始化AipSpeech对象
...
def main_thread():
    ...
    while True:
        ...
        input_words = ""
        for line in iter(input,"不可能的任务"):
            input_words += line
            if " " in input_words or "？" in input_words:
                break
        print("你说："+input_words)
        win32api.keybd_event(F6,0,0,0)
        time.sleep(0.05)
        win32api.keybd_event(F6,0 ,win32con.KEYEVENTF_KEYUP ,0)
